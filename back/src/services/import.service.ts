...
// Columnas esperadas por el sistema para productos
...
// Mapeo de sinónimos comunes para auto-detección
...
interface ColumnMapping {
  [fileColumn: string]: string; // fileColumn -> systemColumn
}
...
class ImportService {
  /**
   * Parsea un archivo CSV/Excel y extrae los headers + datos de preview
   */
  async parseFile(fileBuffer: Buffer, fileName: string, mimeType: string): Promise<ParsedFile> {
    ...
    // Generar datos de preview (primeras 5 filas)
  ...
  /**
   * Sugiere un mapeo automático basado en nombres de columnas
   */
  ...
  async processImport(
    ...
    // Obtener o crear categorías y proveedores existentes
    ...
    for (let rowIndex = 1; rowIndex < data.length; rowIndex++) {
      ...
      try {
        const getValue = (systemKey: string): string => {
          // Buscar qué columna del archivo mapea a este systemKey
          const fileColumn = Object.entries(columnMapping).find(([_, sysKey]) => sysKey === systemKey)?.[0];
        ...
        // SKU: si no se proporciona, generar uno único
        ...
        result.failed++;
        result.errors.push({
          row: rowIndex + 1, // +1 porque rowIndex empieza en 1 (después del header)
    ...
    if (productsToCreate.length > 0) {
      try {
        ...
        // Si falla el bulk, intentar uno por uno para identificar errores
        result.success = 0;
        for (let i = 0; i < productsToCreate.length; i++) {
          try {
            ...
            result.failed++;
            result.errors.push({
              row: i + 2, // +2 por header y índice base 0
  ...
  /**
   * Obtiene las columnas del sistema con sus descripciones
   */
  getSystemColumns() {
